...
`var` 声明的变量是无法通过`delete`删除，继承的属性也无法删除，全局函数也不能删除。但是`this`创建一个可配置的全局属性（没有用`var`）
```javascript
this.x = 1;
delete x; //true
...
`prototype` 中的 `constructor` 不可枚举属性
```javascript
var F = function(){};
var p = F.prototype;
var c = p.constructor;
...
##### 迭代器必须包含`next()`方法，每一次调用都会返回集合的下一个值
```javascript
...
##### 使用[`Symbol.iterator`]结合`for/of`自定义循环迭代
```javascript
  let obj = {
    start: [1, 3, 2],
    end: [7, 9, 8],
...
##### 关于嵌套属性iframe（[点击查看详情](https://www.cnblogs.com/lvhw/p/7107436.html)）
```html
...
##### 关于文档在DOM中的位置坐标（[点击查看原文](https://www.cnblogs.com/youziclub/p/4811069.html)）
...
所谓同源是指：域名，协议，端口均相同
* <span>http://www.123.com/index.html 调用 http://www.123.com/server.php（非跨域）</span>
...
##### 跨域解决方案：（[摘自此处](https://segmentfault.com/a/1190000011145364?utm_source=tag-newest)）
...
#### svg([点击查看原文](http://www.ruanyifeng.com/blog/2018/08/svg.html))
...
#### js中的堆和栈的简单解释（[点击查看原文](https://www.cnblogs.com/jiangk1214/p/6650957.html)）
